...
# ── TP/SL Optimizer (profit maximization) ────────────────────

from tpsl_optimizer import run_full_optimization, run_optimization, evaluate_tpsl, generate_signals_with_paths

_optimization_results: Dict[str, Any] = {}


@app.route("/optimize-tpsl", methods=["POST", "OPTIONS"])
def optimize_tpsl():
    """
    Run full TP/SL parameter optimization across all 37 assets.
    Sweeps ~5,760 parameter combinations to find profitable configurations.
    
    Takes 5-15 minutes. Returns optimal TP/SL multipliers per segment
    (market × direction × regime) with statistical backing.
    
    Request: {"fmp_key": "...", "years": 10}
    Response: Optimal parameters with PF, WR, EV for each segment.
    """
    if request.method == "OPTIONS":
        return "", 204
    if not check_auth():
    ...
    try:
        ...
        logger.info(f"[OPTIMIZE] Starting full TP/SL optimization ({years}yr)")
        result = run_full_optimization(fmp_key, years)
        
        global _optimization_results
        _optimization_results = result
        
        return jsonify(result)
    
    except Exception as e:
        logger.error(f"[OPTIMIZE ERROR] {str(e)}", exc_info=True)
        return jsonify({"error": str(e), "type": type(e).__name__}), 500


@app.route("/optimize-tpsl/quick", methods=["POST", "OPTIONS"])
def optimize_tpsl_quick():
    """
    Quick optimization for a SINGLE asset. Good for testing.
    
    Request: {"asset": "ES", "fmp_key": "...", "years": 10}
    """
    if request.method == "OPTIONS":
        return "", 204
    if not check_auth():
    ...
    try:
        ...
        sigs = generate_signals_with_paths(asset, fmp_key, years)
        if not sigs:
            return jsonify({"error": f"No signals for {asset}"}), 400
        
        buy_sigs = [s for s in sigs if s["signal_type"] == "BUY"]
        sell_sigs = [s for s in sigs if s["signal_type"] == "SELL"]
        
        results = {}
        if len(buy_sigs) >= 20:
            results["buy"] = run_optimization(buy_sigs, f"{asset}_buy")
        if len(sell_sigs) >= 20:
            results["sell"] = run_optimization(sell_sigs, f"{asset}_sell")
        if len(sigs) >= 20:
            results["all"] = run_optimization(sigs, f"{asset}_all")
        ...
        logger.error(f"[QUICK-OPT ERROR] {str(e)}", exc_info=True)
        return jsonify({"error": str(e), "type": type(e).__name__}), 500


@app.route("/optimize-tpsl/results", methods=["GET"])
def optimize_tpsl_results():
    """Return last optimization results."""
    if not check_auth():
        return jsonify({"error": "Unauthorized"}), 401
    if not _optimization_results:
        return jsonify({"status": "no_results"})
    return jsonify(_optimization_results)


# ── Walk-Forward Validation ──────────────────────────────────

from walk_forward import (
    walk_forward_validate, rolling_walk_forward,
    convert_daily_to_hourly, generate_scanner_tpsl_config,
    split_signals_by_date
)

_walk_forward_results: Dict[str, Any] = {}


@app.route("/walk-forward", methods=["POST", "OPTIONS"])
def walk_forward_endpoint():
    """
    Full walk-forward validation: train on 2016-2022, test on 2023-2026.
    Also runs rolling walk-forward across 6 folds.
    Returns verdict: STRONG_EDGE, MARGINAL_EDGE, WEAK_EDGE, or CURVE_FIT.
    
    Takes 5-15 minutes. This is the critical test.
    ...
    try:
        ...
        fmp_key = data.get("fmp_key", os.environ.get("FMP_API_KEY", ""))
        years = data.get("years", 10)
        train_cutoff = data.get("train_cutoff", "2023-01-01")
        ...
        logger.info(f"[WF] Total signals: {len(all_signals)}")
        
        if len(all_signals) < 200:
            return jsonify({"error": f"Too few signals ({len(all_signals)})"}), 400
        
        # Step 2: Fixed-split walk-forward (train before 2023, test after)
        logger.info(f"[WF] Running fixed-split walk-forward (cutoff={train_cutoff})...")
        
        # Run per-segment validation
        ...
        for seg_name, seg_signals in segments.items():
            if len(seg_signals) >= 100:
                logger.info(f"[WF] Validating {seg_name} ({len(seg_signals)} signals)...")
                fixed_results[seg_name] = walk_forward_validate(
                    seg_signals, train_cutoff=train_cutoff
                )
        
        # Step 3: Rolling walk-forward on all signals
        logger.info(f"[WF] Running rolling walk-forward...")
        rolling_result = rolling_walk_forward(all_signals, window_years=4, test_years=1)
        
        # Step 4: Generate hourly conversion config
        deploy_config = _optimization_results.get("deployment_config", {}) if _optimization_results else {}
        overall_wf = fixed_results.get("all", {})
        hourly_config = generate_scanner_tpsl_config(
            {"deployment_config": deploy_config},
            overall_wf
        )
        
        global _walk_forward_results
        ...
        logger.error(f"[WF ERROR] {str(e)}", exc_info=True)
        return jsonify({"error": str(e), "type": type(e).__name__}), 500


@app.route("/walk-forward/results", methods=["GET"])
def walk_forward_results():
    """Return last walk-forward results."""
    if not check_auth():
        return jsonify({"error": "Unauthorized"}), 401
    if not _walk_forward_results:
        return jsonify({"status": "no_results"})
    return jsonify(_walk_forward_results)

...
from historical_backtest import run_historical_backtest, run_full_backtest, ASSET_FMP_SYMBOLS
...
def historical_backtest_asset():
    ...
    try:
        ...
        if asset not in ASSET_FMP_SYMBOLS:
            return jsonify({"error": f"Unknown asset: {asset}", "available": list(ASSET_FMP_SYMBOLS.keys())}), 400

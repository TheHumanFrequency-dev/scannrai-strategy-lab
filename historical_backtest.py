...
# Asset → FMP symbol mapping for HISTORICAL data
# FMP's /historical-price-full/ endpoint does NOT support futures "=F" symbols.
# Solution: Use liquid ETF proxies that track the same underlying.
# ETFs have 10+ years of daily OHLCV on FMP and track futures closely.
# Crypto uses FMP's direct XXXUSD format.
ASSET_FMP_SYMBOLS = {
    # Equity index FUTURES → ETF proxies
    "ES": "SPY",     # S&P 500 → SPDR S&P 500 ETF
    "NQ": "QQQ",     # Nasdaq 100 → Invesco QQQ
    "YM": "DIA",     # Dow Jones → SPDR Dow Jones ETF
    "RTY": "IWM",    # Russell 2000 → iShares Russell 2000
    # Energy FUTURES → ETF proxies
    "CL": "USO",     # Crude Oil → United States Oil Fund
    "NG": "UNG",     # Natural Gas → United States Natural Gas Fund
    "RB": "UGA",     # Gasoline → United States Gasoline Fund
    "HO": "UHN",     # Heating Oil → United States Heating Oil Fund (limited history)
    # Metals FUTURES → ETF proxies
    "GC": "GLD",     # Gold → SPDR Gold Shares
    "SI": "SLV",     # Silver → iShares Silver Trust
    "HG": "CPER",    # Copper → United States Copper Index Fund
    "PL": "PPLT",    # Platinum → abrdn Physical Platinum Shares
    # Bond FUTURES → ETF proxies
    "ZB": "TLT",     # 30yr Bond → iShares 20+ Year Treasury
    "ZN": "IEF",     # 10yr Note → iShares 7-10 Year Treasury
    "ZF": "IEI",     # 5yr Note → iShares 3-7 Year Treasury
    "ZT": "SHY",     # 2yr Note → iShares 1-3 Year Treasury
    "UB": "EDV",     # Ultra Bond → Vanguard Extended Duration
    # Agriculture FUTURES → ETF proxies
    "ZS": "SOYB",    # Soybeans → Teucrium Soybean Fund
    "ZC": "CORN",    # Corn → Teucrium Corn Fund
    "ZW": "WEAT",    # Wheat → Teucrium Wheat Fund
    "ZM": "SOYB",    # Soy Meal → use Soybean proxy (no direct ETF)
    "ZL": "SOYB",    # Soybean Oil → use Soybean proxy
    # Livestock — no liquid ETFs, use broader commodity
    "HE": "COW",     # Lean Hogs → iPath Livestock ETN
    "LE": "COW",     # Live Cattle → iPath Livestock ETN
    # FX FUTURES → ETF proxies
    "6E": "FXE",     # Euro FX → Invesco CurrencyShares Euro
    "6J": "FXY",     # Yen → Invesco CurrencyShares Yen
    "6B": "FXB",     # British Pound → Invesco CurrencyShares GBP
    "NKD": "EWJ",    # Nikkei → iShares MSCI Japan ETF
    # Crypto (FMP uses XXXUSD format for historical data)
...
def fetch_yahoo_daily(symbol: str, years: int = 10) -> Optional[Dict]:
    """Fetch daily OHLCV from Yahoo Finance (free, no API key needed)."""
    try:
        period1 = int(time.time()) - (years * 365 * 86400)
        period2 = int(time.time())
        url = f"https://query2.finance.yahoo.com/v8/finance/chart/{symbol}?interval=1d&period1={period1}&period2={period2}"
        logger.info(f"[YAHOO] Fetching {symbol} ({years}yr daily)...")
        headers = {"User-Agent": "Mozilla/5.0 (compatible)"}
        r = requests.get(url, headers=headers, timeout=30)
        if r.status_code != 200:
            logger.warning(f"[YAHOO] {symbol} HTTP {r.status_code}")
            return None
        data = r.json()
        result = data.get("chart", {}).get("result", [])
        if not result:
            logger.warning(f"[YAHOO] {symbol} no chart result")
            return None
        res = result[0]
        ...
        if not raw_c or len(raw_c) < 100:
            logger.warning(f"[YAHOO] {symbol} insufficient data: {len(raw_c or [])} bars")
            return None
        # Filter nulls
        closes, highs, lows, volumes, dates = [], [], [], [], []
        for i in range(len(raw_c)):
            if raw_c[i] is not None and raw_h[i] is not None and raw_l[i] is not None:
                closes.append(raw_c[i])
                highs.append(raw_h[i])
                lows.append(raw_l[i])
                volumes.append(raw_v[i] if raw_v[i] is not None else 0)
                if i < len(timestamps):
                    from datetime import datetime as dt
                    dates.append(dt.utcfromtimestamp(timestamps[i]).strftime("%Y-%m-%d"))
                else:
                    dates.append(f"bar_{i}")
        logger.info(f"[YAHOO] {symbol} loaded {len(closes)} daily bars ({dates[0] if dates else '?'} to {dates[-1] if dates else '?'})")
...
# Yahoo Finance symbol mapping (different from FMP)
ASSET_YAHOO_SYMBOLS = {
    ...
    "BNB": "BNB-USD", "LINK": "LINK-USD", "ADA": "ADA-USD", "AVAX": "AVAX-USD", "DOGE": "DOGE-USD",
}


def fetch_daily_data(asset: str, fmp_key: str, years: int = 10) -> Optional[Dict]:
    """Try Yahoo first (free, reliable), FMP as fallback."""
    yahoo_sym = ASSET_YAHOO_SYMBOLS.get(asset)
    if yahoo_sym:
        data = fetch_yahoo_daily(yahoo_sym, years)
        if data and len(data["closes"]) >= 100:
            return data
        logger.info(f"[DATA] Yahoo failed for {asset}, trying FMP...")
    
    fmp_sym = ASSET_FMP_SYMBOLS.get(asset)
    if fmp_sym and fmp_key:
        data = fetch_fmp_daily(fmp_sym, fmp_key, years)
        if data and len(data.get("closes", [])) >= 100:
            return data
    
    logger.warning(f"[DATA] No historical data available for {asset}")
    return None


def fetch_fmp_daily(symbol: str, fmp_key: str, years: int = 10) -> Optional[Dict]:
    """Fetch daily OHLCV from FMP (fallback — requires paid plan since Aug 2025)."""
    try:
        ...
        if r.status_code != 200:
            logger.warning(f"[FMP] {symbol} HTTP {r.status_code}: {r.text[:200]}")
            return None
        data = r.json()
        # Log what we got back for debugging
        if isinstance(data, dict):
            logger.info(f"[FMP] {symbol} response keys: {list(data.keys())}")
            # Check for error messages from FMP
            if "Error Message" in data:
                logger.warning(f"[FMP] {symbol} API error: {data['Error Message']}")
                return None
        elif isinstance(data, list):
            logger.info(f"[FMP] {symbol} response is list of length {len(data)}")
        else:
            logger.warning(f"[FMP] {symbol} unexpected response type: {type(data)}")
            return None
        hist = data.get("historical", []) if isinstance(data, dict) else []
        if not hist:
            # Store debug info for the caller
            logger.warning(f"[FMP] {symbol} no historical data. Response preview: {str(data)[:300]}")
            return {"_debug": str(data)[:500], "_url": url.replace(fmp_key, "KEY_HIDDEN"), "closes": [], "highs": [], "lows": [], "volumes": [], "dates": []}
...
def run_historical_backtest(
    ...
    """Run using Yahoo Finance (primary) or FMP (fallback)."""
    start_time = time.time()
    market = ASSET_MARKET.get(asset, "futures")
    
    if asset not in ASSET_YAHOO_SYMBOLS and asset not in ASSET_FMP_SYMBOLS:
    ...
    if not data or len(data.get("closes", [])) < 100:
        return {"error": f"Insufficient data for {asset}", "asset": asset,
                "bars_available": len(data.get("closes", [])) if data else 0}
    
    return run_backtest_from_data(asset, market, data, max_outcome_bars, min_bars_between_signals)


def run_backtest_from_data(
    asset: str, market: str, data: Dict,
    max_outcome_bars: int = 20, min_bars_between_signals: int = 5
) -> Dict:
    """
    Run backtest from pre-fetched OHLCV data.
    This allows the BROWSER to fetch FMP data and send it to Railway.
    """
    start_time = time.time()
    ...
    return {
        "asset": asset,
        "market": market,
        "fmp_symbol": ASSET_FMP_SYMBOLS.get(asset, asset),
        "total_bars": total_bars,
        "date_range": f"{dates[0]} to {dates[-1]}",
        "years": round(total_bars / 252, 1),
        "summary": {
            "total_signals": total,
            "resolved": resolved,
            "won": len(won),
